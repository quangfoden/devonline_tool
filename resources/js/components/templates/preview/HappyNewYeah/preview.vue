<template>
	...
	<div class="container">
		<div class="stage-container remove">
			...
			<div class="wishes-layer" ref="wishesLayer"></div>
			<div class="canvas-container">
				<canvas ref="trailsCanvas"> </canvas>
...
// refs cho canvas & layer
const wishesLayer = ref(null)
...
onMounted(async () => {
  // import động các file JS cũ
  await import('@/assets/js/fscreen.js')
  await import('@/assets/js/MyMath.js')
  const { default: Stage } = await import('@/assets/js/Stage.js')
  await import('@/assets/js/script.js')

  // Khởi tạo Stage (giống HTML cũ)
  ...
  // Nhận data preview từ iframe cha
...
function handlePreviewMessage(e) {
  if (e.data?.type !== 'templatePreviewData') return

  const { messages, images } = e.data.data || {}

  if (Array.isArray(messages)) {
    window.WISH_MESSAGES = messages
  }

  if (Array.isArray(images)) {
    window.imageSources = images
  }
}
</script>
